...
\begin{problem}{4}
    Prove or give a counterexample: If $ V $ is finite-dimensional and 
    $ U $ is a subspace of $ V $ that is invariant under every operator on $ V $,
    then $ U = \{0\} $ or $ U = V $.
\end{problem}

\begin{proof}
    If $ U = \{ 0 \} $, then there's nothing to prove
    ($ T(0) = 0 \in \{ 0 \} $ for any $ T  \in \Lc(V) $).
    Therefore, we can assume that $ U \ne \{ 0 \} $.

    Let $ \dim V = n $. Since $ V $ is finite-dimensional, then $ V \cong \mathbb{F}^{n} $.
    So, we consider $ \mathbb{F}^{n} $ instead of $ V $. Let's denote $ e_1, \ldots, e_{n} $
    the standart basis of $ \mathbb{F}^{n} $. Consider a function $ T_{ij} $
    \begin{gather*}
        T_{ij}(v) = T_{ij}( \sum_{k=1}^{n} a_k e_k) = a_i e_j,
    \end{gather*}
    that is, $ T_{ij} $ just leaves the ith coordinate and moves it to the jth position.
    It is clear, that $ T $ is linear. For example,
    \begin{gather*}
        T_{ij}(u + v) = T( \sum_{k=1}^{n} (a_k + b_k) e_k ) =
        (a_i + b_i) e_j = a_i e_j + b_i e_j = T_{ij}(u) + T_{ij} (v).
    \end{gather*}
    The proof is similar for scalar multiplication.

    Now, let $ u = (a_1, \ldots, a_n) \in U $. Since $ u \ne 0 $, then exists such $ m \leq n $
    that $ a_m \ne 0 $. Then $ \frac{1}{a_m} T_{mj}(u) = e_j $. By hypothesis, 
    $ U  $ is invariant under any operator, in particular, $ T_{mj} $. Therefore,
    $ e_j \in U $ for $ j = 1, \ldots, n $. Then, $ \dim U = \dim V $, hence $ U = V $.
\end{proof}
...
\begin{problem}{8}
Suppose $ P \in \Lc(V) $ 
is such that $ P^{2} = P $. Prove that if $ \lambda $ is an eigenvalue of $ P $,
then $ \lambda = 0 $ or $ \lambda = 1 $.    
\end{problem}

\begin{proof}
    Let $ v \ne 0 $ be the corresponding eigenvector. Then
    \begin{gather*}
        P^2 v = P v = \lambda v \\ 
        P^2 v = P(P v) = P (\lambda v) = \lambda P v = \lambda^{2} v.
    \end{gather*}
    So $ \lambda = \lambda^{2} $, therefore $ \lambda = 0 $ or $ \lambda = 1 $.
...
\begin{proof}
%Let $ u \in V $ be a corresponding eigenvector. Let $ u = (a_1, \ldots, a_n) $ its
%coordinates in the given basis. Since $ u \ne 0 $, then some $ a_k \ne 0 $. Then 
%\begin{gather*}
%    Tu = T( \sum_{i=1}^{n} a_i v_i)  =  \sum_{i=1}^{n} a_i T(v_i) = \sum_{i=1}^{n}
%    a_i \lambda v_i \implies \\ 
%    T v_k = \lambda v_k.
%\end{gather*}
%
We know that 
...
\begin{proof}
    Suppose $ Tu = \alpha u, Tw = \beta u, T(u+w) = \lambda(u + w) $. Then
    \begin{gather*}
        \alpha u + \beta w = \lambda u + \lambda w \\ 
        (\alpha - \lambda) u = (\lambda - \beta) w.
    \end{gather*}

    If $ \lambda = \beta $, then $ (\alpha - \lambda) u = 0 $, since
    $ u \ne 0 $, therefore $ \alpha - \lambda = 0 $, hence $ \alpha = \lambda = \beta $.
    In the same fashion, if $ \lambda = \alpha $, then $ \alpha = \beta $.

    Assume $ \lambda \ne \alpha $. Then 
    \begin{gather*}
        u = \frac{\lambda - \beta}{\alpha - \lambda} w \\ 
        T(u+w) = T(\frac{\lambda - \beta}{\alpha - \lambda} w +
        \frac{\alpha - \lambda}{\alpha - \lambda} w) =
        T(\frac{\alpha - \beta}{\alpha - \lambda} w) =
        \beta \frac{\alpha - \beta}{\alpha - \lambda} w \\ 
        T(u+w) =  \lambda(u + w) = \lambda \frac{\alpha - \lambda}{\alpha - \lambda} w.
    \end{gather*}
    Comparing last two identities we get $ \beta = \lambda $, therefore $ \alpha = \beta $.
...
\begin{proof}
Consider $ V = \mathbb{R}^{2} $ and $ T $ -- rotation by $ 90^{ \circ} = \frac{\pi}{2} $.
Then in the standard basis (with a slight abuse of notation)
\begin{gather*}
    T =
    \begin{pmatrix}
    ...
    \end{pmatrix} .
\end{gather*}
Easy to see that $ T^{2} = -I \iff T^{2} + I = 0 $.
Note that $ T^{2} $ has an upper-triangular matrix (since a diagonal matrix is a special case
thereof).
From geometrical considerations it is easy
to see that $ T \ne cI $. Therefore $ p(z) = z^{2} +1 $ is a minimal polynomial of $ T $.
It has no real roots (the roots are $ \pm i $), so $ p(z) $ cannot be decomposed into a product 
of monomials. So, by the previous result, $ T $ has no upper-triangular matrix.
Hence, we constructed a counterexample.
...
\begin{gather*}
    ...
    \end{pmatrix}.
\end{gather*}
Then $ T^{2} = - I $, therefore $ T^4 = I $. So $ T T^{3} = I $.
\end{proof}
...
\begin{gather*}
    ...
    \end{pmatrix},
\end{gather*}
for any $ a \ne 0 \in \mathbb{F} $. Therefore $ T $ is not injective, hence
...
\begin{problem}{2}
    Suppose $ T \in \mathcal{L}(V) $ has a diagonal matrix $ A $ with respect to some basis of 
    $ V $. Prove that if $ \lambda \in \mathbb{F} $, then $ \lambda $ appears
    on the diagonal of $ A $ precisely $ \dim E(\lambda, T) $ times.
\end{problem}

\begin{proof}
    First, if $ \dim E(\lambda, T) = 0 $, then $ \lambda $ does not appear on the diagonal.
    So we assume that $ \lambda $ is an eigenvalue of $ T $.
    
    Suppose $ v_1, \ldots, v_n $ is the basis w.r.t which $ A $ is diagonal.
    Then all of them are eigenvectors of $ T $.
    Assume that $ \lambda $ appears $ k $ times on the diagonal of $ A $.
    WLOG let us reindex $ \{ v_i \} $ s.t. $ v_{1}, \ldots, v_{k} $ are
    eigenvectors corresponding to the 
    eigenvalue $ \lambda $
    (we can always restore original indices of $ v_i $).
    Also, let $ m = \dim E(\lambda, T) $. 

    Clearly, $ k \leq m $, since $ v_{1}, \ldots, v_{k} $ are linearly independent
    and they all are in $ E(\lambda, T) $. Now assume $ k < m $.
    Then exists $ u \ne 0 \in E(\lambda, T) $ s.t. 
    $ u \not \in span(v_1, \ldots, v_k) $.
    Let
    \begin{gather*}
        u = a_1 v_1 + \ldots + a_n v_n = a_{k+1} v_{k+1} + \ldots + a_{n} v_n
    \end{gather*}
    be the decomposition of $ u $ w.r.t to the basis.
    Then
    \begin{gather*}
        0 = Tu - T \sum_{j=k+1}^{n} a_{j} v_{j} =
        \lambda u - \sum_{j=k+1}^{n} \mu_{j} a_{j} v_{j}
        = 
        \\ \sum_{j=k+1}^{n} \lambda a_{j} v_{j} - \sum_{j=k+1}^{n} \mu_{j} a_{j} v_{j}
        = \sum_{j=k+1}^{n} (\lambda - \mu_j) a_{j} v_{j},
    \end{gather*}
    where  $ \mu_j $ is the eigenvalue corr. to $ v_j $. Since $ v_j $ are all 
    linearly independent, then $ \lambda = \mu_j $ which leads to contradiction.
    Therefore, $ k = m $.
\end{proof}
